...
function runStopaganda(){
  ...
  function run(sourceHash){
    ...
    if(tab.length == 0){
      ...
      // Default tab
      var linkClass = ".js-result-extras-url";
      ...
      var standardLinks = standardArray.map(function(e){ return [e.closest('.js-result-extras'), e.textContent.match(linkRegex)[1], e.textContent, false] }).filter(function(e) { return e[0] });
      ...
      var baseLinks = vidArray.map(function(e){ return [e.querySelectorAll('.t-m')[e.querySelectorAll('.t-m').length - 1], e.getAttribute('data-link').match(linkRegex)[1], e.getAttribute('data-link'), false, true] }).filter(function(e) { return e[0] });
  ...
  // set observing var to help frequency of observations possible
  var observing = false;

  // Wait until page is fully loaded then define observer
  function initObserver(){
    // targetNode is element with list of cards (tweets) displayed
    var targetNode = document.querySelector('.results');
    if(!targetNode || targetNode.childElementCount == 0){ 
    ...
    var observer = new MutationObserver(function(mutationsList, observer){
      for(var mutation of mutationsList){
        // only run if not within 250 ms of running
        if(!observing){
          observing = true;
          run(sourceHash);
        }
