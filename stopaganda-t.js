...
chrome.storage.sync.get('stopagandaSettings', function(results){
  if(!results['stopagandaSettings']){
    ...
    showThreads = true;
  }else if(results['stopagandaSettings']['threads'] == undefined){
    // default to showing if Threads is not set
    showThreads = true;
  }else{
    showThreads = results['stopagandaSettings']['threads']
  }
  if(showThreads){
    runStopaganda();
  }
...
function runStopaganda(){
  ...
  function run(sourceHash){
    ...
    var sourcesArray = Array.from(document.querySelectorAll(linkContainer)).map(function(e) { return e.querySelector(linkSelector + ':not(.stopaganda)') })
